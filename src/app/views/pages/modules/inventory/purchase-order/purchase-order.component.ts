...
import {
    SnackbarModel, AlertTypeClass, AlertType, SnackToastService, DeleteConfirmComponent
...
export class PurchaseOrderComponent implements AfterViewInit, OnDestroy {
    ...
    displayedColumns = ['select', 'id', 'storeName', 'supplierName', 'transactionDate', 'orderStatus',
        'totalAmount', 'action'];
    ...
    onAction(id: number) {
        let instance: MatDialogRef<PurchaseOrderFormComponent, any>;
        // let instance: MatDialogRef<DartaFormComponent, ResponseModel>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(PurchaseOrderFormComponent, {
            width: '800px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    private initData() {
        ...
        this.pService.getList().pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.pService.deleteList(id)
