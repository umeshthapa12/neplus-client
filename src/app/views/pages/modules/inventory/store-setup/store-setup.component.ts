...
export class StoreSetupComponent implements AfterViewInit, OnDestroy {
    private readonly toDestroy$ = new Subject<void>();

    displayedColumns = ['select', 'id', 'storeCode', 'storeName', 'branchName', 'status', 'action'];
    ...
    filters(e) {
        this.options = e;
        let allValue: any[];
        this.storeSetupService.getList().subscribe({
            next: res => {
                allValue = res;
            }
        });
    ...
    onAction(id: number) {
        let instance: MatDialogRef<StoreSetupFormComponent, any>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(StoreSetupFormComponent, {
            width: '700px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    private initData() {
        ...
        this.storeSetupService.getList().pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.storeSetupService.delete(id)
                    .pipe(takeUntil(this.toDestroy$)))
                // for real data
