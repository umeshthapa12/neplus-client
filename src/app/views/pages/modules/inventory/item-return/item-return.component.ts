...
export class ItemReturnComponent implements OnDestroy, AfterViewInit {

    private readonly toDestroy$ = new Subject<void>();

    displayedColumns = ['select', 'id', 'transactionDate', 'returnToStore', 'returnBy', 'remarks', 'sendNotification',
        'status', 'action'];
    ...
    filters(e) {
        this.options = e;
        let allValue: any[];
        this.itemReturnService.getList().subscribe({
            next: res => {
                allValue = res;
            }
        });
    ...
    onAction(id: number) {
        let instance: MatDialogRef<ItemReturnFormComponent, any>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(ItemReturnFormComponent, {
            width: '600px',
            data: data ? data : {},
            autoFocus: false,
    ...
    deleteSelected() {
        ...
        instance.afterClosed().pipe(takeUntil(this.toDestroy$), filter(_ => _)).subscribe(_ => {
            ...
            // this.itemReturnService.deleteSelected(this.selection.selected)
            //     .subscribe(_ => {
            //         console.log(_);
    ...
    private initData() {
        ...
        this.itemReturnService.getList().pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.itemReturnService.delete(id)
