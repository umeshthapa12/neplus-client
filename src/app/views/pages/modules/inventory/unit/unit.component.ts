...
export class UnitComponent implements AfterViewInit, OnDestroy {
    private readonly toDestroy$ = new Subject<void>();

    displayedColumns = ['select', 'sn', 'unitName', 'status', 'action'];
    ...
    filters(e) {
        this.options = e;
        let allValue: any[];
        this.unitService.getList().subscribe({
            next: res => {
                allValue = res;
            }
        });
    ...
    onAction(id: number) {
        let instance: MatDialogRef<UnitFormComponent, any>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(UnitFormComponent, {
            width: '600px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    private initData() {
        ...
        this.unitService.getList().pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.unitService.delete(id)
