...
import { takeUntil, filter, delay, switchMap, debounceTime, tap } from 'rxjs/operators';
...
import {
    ParamGenService, ExtendedMatDialog, CustomAnimationPlayer, collectionInOut, fadeInOutStagger
} from '../../../../../../utils';
import { Store } from '@ngxs/store';
import { BasicInfoFormComponent } from './basic-info-form.component';
import { BasicInfoService } from './basic-info-service';
import { LoadLazyEmployeeAction } from './store/employee.store';
...
export class BasicInfoComponent implements AfterViewInit, OnDestroy {
    private readonly toDestroy$ = new Subject<any>();

    displayedColumns = [
        'details', 'select', 'id', 'empCode', 'title', 'firstName',
        'gender', 'nationality', 'mobileNumber', 'status', 'action'
    ];
    ...
    onAction(id: number) {
        let instance: MatDialogRef<BasicInfoFormComponent, any>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(BasicInfoFormComponent, {
            width: '900px',
            data: data ? data : {},
            autoFocus: false,
    ...
    private initData() {
        ...
        this.basicInfoService.getList().pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.basicInfoService.delete(id)
