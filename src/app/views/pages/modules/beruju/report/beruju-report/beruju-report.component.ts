...
@Component({
    templateUrl: './beruju-report.component.html',
    animations: [fadeIn, fadeInOutStagger, collectionInOut],
    styleUrls: ['./beruju-report.scss'],
})

export class BerujuReportComponent implements AfterViewInit, OnDestroy {
    private readonly toDestroy$ = new Subject<void>();

    displayedColumns = ['select', 'id', 'name', 'totalMoney', 'persuadeMoney', 'balance', 'action'];
    ...
    onAction(id: number) {
        // let instance: MatDialogRef<BerujuReportFormComponent, ResponseModel>;
        let instance: MatDialogRef<BerujuReportFormComponent, any>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(BerujuReportFormComponent, {
            width: '650px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                // switchMap(() => this.brService.deleteBerujuReport(id)
                //     .pipe(takeUntil(this.toDestroy$)))
                switchMap(() => this.brService.deleteList(id)
                    .pipe(takeUntil(this.toDestroy$)))
            )
            .subscribe({
                next: res => {
    ...
    private initData() {
        ...
        // Real data
        // this.brService.getBerujuReports(this.query).pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
        //     this.cdr.markForCheck();
        ...
        this.brService.getList().pipe(takeUntil(this.toDestroy$), delay(1000)).subscribe(res => {
            this.cdr.markForCheck();
